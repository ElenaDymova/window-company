...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    //навесили на каждую форму внутри обработчик события
    item.addEventListener('submit', function (e) {
      //на каждый айтем навесили обраотчик событий submit, внутри объект события
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); //отслеживаем, в какой таб кликнул пользователь, используем делегирование событий

  header.addEventListener('click', function (e) {
    var target = e.target; //переменная с элементом, на кот. произошло событие (куда кликнул пользователь)
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || //проверяем тоже самое только у родителя
    //определяем в какой именно по счету элемент кликнули
    target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      tab.forEach(function (item, i) {
        //item каждый элемент кот перебираем, i - номер перебираемого элемента
        if (target == item || target.parentNode == item) {
          //если тот элемент в кот кликнул пользователь будет равен 
